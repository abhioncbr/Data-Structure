...
public class BinaryTreePrinter<T extends Comparable<T>, E> {

    public void printNode(Node<T,E> root) {
        int maxLevel = maxLevel(root);

        printNodeInternal(Collections.singletonList(root), 1, maxLevel);
    }

    private void printNodeInternal(List<Node<T,E>> nodes, int level, int maxLevel) {
    ...
    private int maxLevel(Node<T,E> node) {
        if (node == null)
            return 0;
    ...
    private boolean isAllElementsNull(List<Node<T,E>> list) {
